...
const statusActions = computed(() => {
  ...
  if (s === 'open') {
    actions.push({ label: 'Взять в работу', status: 'in_progress', class: 'btn--primary' })
    actions.push({ label: 'Ожидает', status: 'waiting', class: 'btn--secondary' })
  }
  if (s === 'waiting' || s === 'in_progress') {
    actions.push({ label: 'Решён', status: 'resolved', class: 'btn--success' })
    actions.push({ label: 'Вернуть в работу', status: 'in_progress', class: 'btn--secondary' })
  }
  if (s !== 'closed') {
    actions.push({ label: 'Закрыть', status: 'closed', class: 'btn--danger' })
  }
...
const changeStatus = async (status: string) => {
  ...
  try {
    ...
    if (ticket.value) {
      ticket.value.status = status as Ticket['status']
    }
  } catch (e) {
...
const onTyping = () => {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({
      ...
      userName: authStore.user?.full_name || 'Администратор',
    }))
  }
...
onUnmounted(() => {
  if (ws) {
    ws.send(JSON.stringify({ type: 'leave', ticketId: route.params.id }))
    ws.close()
  }
  if (typingTimeout) clearTimeout(typingTimeout)
})
...
<template>
  <section class="ticket-detail">
    <NuxtLink to="/admin/support" class="back-link">
      ...
      <div class="ticket-panels">
          ...
          <div class="chat-header">
            <div class="ticket-info">
              <div class="ticket-info__top">
                ...
                <span class="ticket-status" :class="statusConfig[ticket.status]?.class">
              ...
              <div class="ticket-meta">
                ...
                <span v-if="ticket.closedAt">•</span>
                <span v-if="ticket.closedAt">
                  Закрыт: {{ formatDate(ticket.closedAt) }}
                </span>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="message__content">
                <div class="message__header">
                  ...
                  <span v-if="msg.userRole === 'admin'" class="message__role">Поддержка</span>
                  <span v-if="msg.userRole === 'customer'" class="message__role message__role--user">Клиент</span>
                ...
                <div v-if="msg.attachments?.length" class="message__attachments">
                  <a
                    ...
                    :key="att.id"
                    :href="`/api/tickets/files/${att.fileName}`"
                    target="_blank"
                    class="attachment"
                  >
        ...
        <aside class="ticket-sidebar">
          <div class="sidebar-block">
            <h3>Клиент</h3>
            <div class="user-card">
              <div class="user-card__avatar">{{ getInitial(ticket.userName) }}</div>
          ...
          <div class="sidebar-block">
            ...
            <div class="info-list">
              <div class="info-row">
                <span>Категория</span>
                <strong>{{ categoryLabels[ticket.category] || ticket.category }}</strong>
              </div>
              <div class="info-row">
                <span>Приоритет</span>
              ...
              <div class="info-row">
                <span>Сообщений</span>
                <strong>{{ ticket.messages?.length || 0 }}</strong>
              </div>
            </div>
          </div>

          <div v-if="statusActions.length" class="sidebar-block">
            <h3>Управление статусом</h3>
            <div class="status-actions">
              <button
                v-for="action in statusActions"
                :key="action.status"
                :class="['btn-status', action.class]"
    ...
    <div v-else class="error-state">
      ...
      <NuxtLink to="/admin/support">Вернуться к списку</NuxtLink>
    </div>
  </section>
</template>
...
.message {
  ...
  &--mine {
    ...
    .message__author { color: rgba(255,255,255,0.8); }
    .message__role { background: rgba(255,255,255,0.2); color: #fff; }
    .message__time { color: rgba(255,255,255,0.7); }
    .message__text { color: #fff; }
    .message__avatar { background: $green; color: #fff; }
  }
}
...
.attachment__icon { font-size: 1.4rem; }
.attachment__name { color: #2d2d2d; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.attachment__size { color: #b0b0b0; font-size: 1.1rem; }
...
.chat-closed {
  ...
  p { color: #9a9a9a; font-size: 1.4rem; }
}
