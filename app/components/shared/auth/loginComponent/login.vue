...
async function handleSubmit() {
  ...
  catch { formError.value = 'Неверный e-mail или пароль' }
  ...
  <main class="auth-page"><section class="auth-panel">
    ...
    <div class="auth-panel__content">
      ...
      <form class="auth-panel__form" @submit.prevent="handleSubmit">
        <div class="auth-field"><UInput v-model="email" type="text" placeholder="E-mail" /><p v-if="fieldErrors.email" class="auth-field__error">{{ fieldErrors.email }}</p></div>
        <div class="auth-field"><UInput v-model="password" type="password" placeholder="Пароль" /><p v-if="fieldErrors.password" class="auth-field__error">{{ fieldErrors.password }}</p></div>
        <p v-if="formError" class="auth-panel__error">{{ formError }}</p>
        <button type="submit" class="auth-panel__submit" :disabled="isSubmitting" :class="{ 'auth-panel__submit--loading': isSubmitting }"><span v-if="isSubmitting" class="spinner" /><template v-else>Продолжить</template></button>
      </form>
      <p class="auth-panel__footer">Нет аккаунта? <NuxtLink to="/auth/signin">Зарегистрируйтесь</NuxtLink></p>
    </div>
  </section></main>
</template>
...
.auth-panel__logo { display: inline-flex; margin-bottom: 6.2rem; width: 100%; img { width: 50%; } }
